...
class SubAdapter:RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when(viewType){
            ...
            BaseType.TYPE_TopRated->{
                (holder as TopRatedMovieViewHolder).bindData(data[position] as com.rxyzent.homework56.model.response.home.topRated.Result)
                holder.binding.root.setOnClickListener {
                    onSubItemClik?.let {
                        it.onSubClick((data[position] as com.rxyzent.homework56.model.response.home.topRated.Result).id)
                    }
                }
            }
            BaseType.TYPE_Upcoming->{
                (holder as UpcomingMovieViewHolder).bindData(data[position] as com.rxyzent.homework56.model.response.home.upcoming.Result)
                holder.binding.root.setOnClickListener {
                    onSubItemClik?.let {
                        it.onSubClick((data[position] as com.rxyzent.homework56.model.response.home.upcoming.Result).id)
                    }
                }
            }
            BaseType.TYPE_NowPlaying->{
                (holder as NowPlayingMovieViewHolder).bindData(data[position] as com.rxyzent.homework56.model.response.home.nowPlaying.Result)
                holder.binding.root.setOnClickListener {
                    onSubItemClik?.let {
                        it.onSubClick((data[position] as com.rxyzent.homework56.model.response.home.nowPlaying.Result).id)
                    }
                }
            }
        }
